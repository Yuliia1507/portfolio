...
document.addEventListener("DOMContentLoaded", function () {
	const elementsToObserve = document.querySelectorAll('.title, .text, .info-service__item, .service__image, .projects__item');
...
document.addEventListener("DOMContentLoaded", function () {
	...
	// Функція для переключення активної колекції та кнопок
	function toggleCollection(collectionToShow, collectionToHide, btnToActivate, btnToDeactivate) {
		...
		activeCollection = collectionToShow;
	...
	// Функція для показу/приховування елементів в активній колекції
	function toggleItems() {
		...
		// Оновлення тексту кнопки на основі кількості видимих елементів
		showMoreBtn.textContent = itemsShown >= items.length ? "Hide" : "See All";
	}

	// Ініціалізація видимості активної колекції
	toggleItems();

	showMoreBtn.addEventListener("click", function () {
		const items = activeCollection.querySelectorAll(".projects__item");

		// Перевірка, чи всі елементи показані, щоб повернутись до початкового стану
		if (itemsShown >= items.length) {
			itemsShown = itemsToShow; // Повертаємо початкову кількість елементів
			toggleItems();
			showMoreBtn.scrollIntoView({ behavior: "smooth", block: "center" }); // Прокрутка до кнопки
		} else {
			itemsShown += itemsToShow; // Показуємо ще 3 елементи при кожному натисканні
			toggleItems();
		}
	});
